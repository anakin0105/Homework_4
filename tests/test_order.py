...
def test_order_str(order):
    ...
    assert str(order) == "Заказ: Order1, Товар: Phone, Количество: 2, Итоговая стоимость: 100000.0 руб."

def test_order_invalid_product():
    """Проверка ошибки при передаче некорректного товара."""
    with pytest.raises(TypeError, match="Товар должен быть экземпляром класса, наследующего BaseProduct"):
